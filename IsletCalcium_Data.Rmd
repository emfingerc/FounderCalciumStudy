...
## Calcium P-Value Summaries

```{r}
if(file.exists(rdsfile <- "calciumPvalue.rds")) {
  calciumPvalue <- readRDS(rdsfile)
} else {
  calciumPvalue <- broomit(
    calciumData,
    signal = "strain * condition * sex",
    ancillary = "strain * sex + condition * sex") %>%
    arrange(p_signal)
  
  saveRDS(calciumPvalue, rdsfile)
}
rm(rdsfile)
```
...
```{r}
if(file.exists(rdsfile <- "calciumCondPvalue.rds")) {
  calciumCondPvalue <- readRDS(rdsfile)
} else {
  calciumCondPvalue <- broomit(
    calciumCondData,
    signal = "strain * sex",
    ancillary = "sex") %>%
    arrange(p_signal)
  
  saveRDS(calciumCondPvalue, rdsfile)
}
rm(rdsfile)
```
...
if(file.exists(rdsfile <- "proteinPvalue.rds")) {
  proteinPvalue <- readRDS(rdsfile)
} else {
  proteinPvalue <-
    broomit(
      proteinData,
...
```{r}
traitPvalue <- bind_rows(
  calcium = readRDS("calciumPvalue.rds"),
  protein = readRDS("proteinPvalue.rds"),
  .id = "datatype")
...
```{r}
traitCondPvalue <- bind_rows(
  calcium = readRDS("calciumCondPvalue.rds"),
  protein = readRDS("proteinPvalue.rds"),
  .id = "datatype")

saveRDS(traitCondPvalue, "traitCondPvalue.rds")
rm(traitCondPvalue)
```
...
corProteinCalciumMean <- cor(
  proteinSignal %>%
    ...
    select(proteinPvalue$trait),
  calciumCondSignal %>%
    filter(!(strain == "NZO" & sex == "M")) %>%
    arrange(trait, strain, sex) %>%
    select(-signal) %>%
    pivot_wider(names_from = "trait", values_from = "mean") %>%
    select(calciumCondPvalue$trait),
  use = "pair")
```
