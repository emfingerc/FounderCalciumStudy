...
traitStats <- bind_rows(
  ...
  calcium8G = readRDS("calciumStats.rds"),
  .id = "datatype")
```
...
```{r}
dropTraits <- unique(filter(traitStats, is.na(p.value)))$trait
```
...
traitData <- bind_rows(
  ...
  calcium8G = readRDS("calciumData.rds"),
  .id = "datatype")
...
traitSignal <- bind_rows(
  ...
  calcium8G = readRDS("calciumSignal.rds"),
  .id = "datatype"
)
